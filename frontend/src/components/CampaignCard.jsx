...
const CampaignCard = ({ campaign = {} }) => {
  ...
  return (
    <Card className="overflow-hidden border-none shadow-[0px_10px_30px_-5px_rgba(69,87,59,0.08)] hover:shadow-[0px_20px_40px_-10px_rgba(69,87,59,0.12)] transition-shadow duration-500 bg-sage-50 rounded-bl-[24px] rounded-br-[40px] rounded-tl-[32px] rounded-tr-[16px] flex flex-col h-full group">
      {/* Clickable Image and Content Section */}
      <Link to={`/campaigns/${_id || ''}`} className="flex flex-col flex-grow group/link">
        {/* Image Section */}
        <div className="relative h-60 overflow-hidden">
          <img
            ...
            loading="lazy"
            className="w-full h-full object-cover transition-transform duration-700 group-hover/link:scale-110 group-hover:scale-110"
          />
        ...
        <CardHeader className="p-8 pb-3 flex-grow">
          <h3 className="text-2xl font-fraunces font-thin text-sage-800 line-clamp-1 leading-tight group-hover/link:text-earth-500 transition-colors duration-300" title={title}>
