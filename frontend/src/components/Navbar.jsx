              ...
              <div className="relative">
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-sage-200 border-2 border-white shadow-sm overflow-hidden flex items-center justify-center">
                    {user.avatar ? (
                      <img src={user.avatar} alt="User profile avatar" className="w-full h-full object-cover" />
                    ) : (
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-sage-800/10 py-2 animate-in fade-in zoom-in-95 duration-200">
                    <div className="px-4 py-3 border-b border-sage-800/5 mb-1">
                      <p className="text-sm font-bold text-sage-800 truncate">{user.name || "User"}</p>
                      <p className="text-[11px] text-sage-800/60 truncate">{user.email || ""}</p>
                    </div>
                    <Link 
                      to="/dashboard" 
              ...
              {user ? (
                <>
                  ...
                  <Link to="/dashboard" onClick={() => setIsMobileMenuOpen(false)}>
                    <Button variant="ghost" className="w-full py-6 text-sage-800 rounded-full font-bold flex items-center gap-2">
